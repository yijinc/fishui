...
import { reactive, defineProps, withDefaults, defineEmits, defineExpose, getCurrentInstance } from 'vue';
...
const tryScrollToLower = ({ detail: { scrollTop, scrollHeight } }: any) => {
  /**
...
onReady(() => {
  uni.createSelectorQuery().in(getCurrentInstance()).select(`#${props.id}`).boundingClientRect((res) => {
    _scrollHeight = (res as UniApp.NodeInfo).height!;
  }).exec();
});
